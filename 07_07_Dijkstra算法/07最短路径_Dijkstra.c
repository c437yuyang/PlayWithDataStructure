...
void ShortestPath_Dijkstra(MGraph G, int v0, Patharc *P, ShortPathTable *Dist)
{
	...
	for (v = 0; v < G.numVertexes; v++)    /* 初始化数据 */
	{
		...
		(*Dist)[v] = G.arc[v0][v];/* 将与v0点有连线的顶点加上权值 */
		(*P)[v] = /*0*//*v0*/-1;//为-1表示还没找到或者不需要中间节点				/* 初始化路径数组P为0  *///最开始就是假设所有顶点都是直接到v0最近,我自己改的，原作者应该是写错了，写的0
	}
...
void ShortestPath_Dijkstra1(MGraph G, int v0, Patharc *P, ShortPathTable *Dist)
{
	...
	{
		(*Dist)[i] = G.arc[v0][i];
		(*P)[i] = -1; //所有节点前驱初始化成-1，表示未找到(可以理解为就是直达)
	}
	(*Dist)[v0] = 0;
	final[v0] = 1;

	for (int i = 1; i < G.numVertexes; ++i) //循环这么多次
	{
